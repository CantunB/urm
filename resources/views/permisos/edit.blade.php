...
<div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                <h2 style="text-transform:uppercase;" class="header-title"><strong>Asignar permisos para {{$user->name}}</strong></h2>
                <p class="sub-header">
          ...
          > Acceso Total <span><strong>( Seleccionar todo los permisos existentes)</strong></span></p>
                ...
                <div class="container">
                    <form action="{{ action('PermissionController@update', $user->id) }}" method="POST" class="form-group">
                        @csrf
                        @method('PUT')
                                        ...
                                        @if($item < "4")
                                                ...
                                                <label>
                                                    <input type="checkbox"
                                                        class="chkusers case"
                                                        name="permission[]"
                                                        value="{{ $val->id }}" {{ $user->permissions->pluck('id')->contains($val->id) ? 'checked' : '' }}
